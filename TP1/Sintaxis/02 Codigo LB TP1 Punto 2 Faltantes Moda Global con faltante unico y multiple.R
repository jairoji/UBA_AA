...
for(j in 2:length(Faltantes)){
  ...
  base.final.training = Imputar.Moda.Fun.Factores(base.final.training)
...
Conf.VS.Faltantes = ggplot(data = info.confidence.multiple.melt1, aes(x = Faltantes, y = Porcentaje, 
                                                                     group = interaction(Confianza, Característica), linetype = Característica, colour = Confianza)) + geom_line(size = 1.2)
Conf.VS.Faltantes
